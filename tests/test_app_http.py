...
class AppHttpTests(unittest.TestCase):
    ...
    def test_push_then_has_source_then_preflight_ready(self):
        ...
        self.assertEqual(preflight("127.0.0.1", url, self.app.ws_port)["state"],
                         "missing")

        ok, err = push_source("127.0.0.1", make_source(url), self.app.ws_port)
    ...
    def test_preflight_unreachable_does_not_raise(self):
        ...
        result = preflight("127.0.0.1", "https://t.example/", dead)
        self.assertEqual(result["state"], "unreachable")
        self.assertTrue(result["error"])

    def test_preflight_without_host_or_url(self):
        self.assertEqual(preflight("", "https://t.example/")["state"], "unreachable")
    ...
    def test_preflight_uses_raw_url_not_normalized(self):
        ...
        self.assertEqual(
            preflight("127.0.0.1", "https://t.example", self.app.ws_port)["state"],
            "ready")
        self.assertEqual(
            preflight("127.0.0.1", "https://t.example/", self.app.ws_port)["state"],
            "missing")
