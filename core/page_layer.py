...
PAGE_MARKERS = [
    ("长 base64 赋值", re.compile(r"""var\s+\w+\s*=\s*['"][A-Za-z0-9+/=]{200,}['"]"""),
...
def _gather(text: str, markers) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for why, rx, note in markers:
        m = rx.search(text)
        if not m:
            continue
        out.append({"why": why, "note": note,
                    "snippet": m.group(0)[:120].replace("\n", " ").strip(),
                    "line": _line_of(text, m.start())})
    return out


def classify_page(html: str, want: str = "") -> Dict[str, Any]:
    """判**这一页**属于哪一层（页面侧）。返回：

    - ``layer``：``"L1"`` / ``"L2"`` / ``"L3"`` / ``"L4"`` / ``""``（判不了——既没有目标定义、
      也没有任何痕迹时不硬猜；``unsure`` 里写明为什么）
    - ``evidence``：证据行（``{why, snippet, line, note}``）
