// 校验参数的字段定义：**唯一一份**。
//
// 现在只有**一处**渲染它：设置抽屉里的「校验」面板（`CheckSettingsPanel` →
// `CheckFieldRow`），外加列表页从 `DEPTH_SHORT` 取深度简称。**本次覆盖那套参数不在这里**
// ——那是本机引擎的参数（见 `jvmDepth.js` + `CheckJvmForm.vue`），两套字段不是一回事。
// 历史上这里供两个组件渲染，字段数分家过（8 vs 6）；一份定义这个规矩就是那时候立的。
//
...
//
// ⚠️ **主页档（1）已从这个下拉里撤掉，但标签留着**：值 1 仍然合法——历史结论里存着它、
// 迁移「没开搜索探测」的旧配置也落在它上面，而列表页那条 `DEPTH_SHORT[probe_depth]`
// 还要靠它把老结论的档位写出来。**候选值那份在 `core/settings_store.PROBE_DEPTH_CHOICES`**
// ——别照这份表去渲染下拉（下拉的选项来自后端 limits，AGENTS #8）。
