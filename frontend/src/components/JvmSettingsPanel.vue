<template>
  <div class="jvm-panel">
    ...
    <!-- 跑批入口**不在这里**：结论落在列表页的 JVM 列，所以触发也在那一页
         （全量校验弹框 → 引擎选「本机引擎」）。这里只配参数 + 看最近一次的结果。 -->
    <el-alert type="info" :closable="false" show-icon style="margin-bottom: 12px"
              title="跑批入口在书源列表的「全量校验」弹框里，引擎选「本机引擎」">
      <template #default>
        <div style="font-size: 12px">
          本页只管配置参数；改完记得点「保存参数」，跑批读的就是这里的值。
        </div>
      </template>
    </el-alert>
...
//    结论落在 `meta`、显示在书源列表的 JVM 列，所以触发也在那一页的「全量校验」弹框里
