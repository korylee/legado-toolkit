...
**校验走本机引擎**：「全量校验」/「校验选中」/ **列表行的「校验」按钮**都在 JVM 里跑
「阅读」App 的真源码（单条与批量给的是同一个结果体，界面上看不出区别）。**代价**：本机
引擎环境从「可选」变成**校验的前提**——没配 App 源码目录就没有校验入口（调试同理）。
多条与全量那条路暂时还留着本地引擎的入口，等本地执行体整体退役时一并撤掉。结论落在两处：健康档位 / 星级 / 深度进 `checks` 表，逐段明细（命中的书、
