...
### 第二层：健康度（联网校验结果）
五档按**下一步动作**划分——两档该不该并成一档，看动作是否相同：
...
> 「证书问题」这一档 2026-09-20 撤掉了：校验收成 App 引擎之后它**产不出来**——App 要么
> 直接通过（它不校验证书信任链），要么报 TLS 阻断（→ 需翻墙）。「证书不被信任」这个
> **原因**仍然留着，写在失败原因里；旧分组里的这个词由换词表迁移成「待验证」。
