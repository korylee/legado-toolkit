...
>
> **本轮已修**（2026-09-16；机制见 lessons §十八/§十九/§二十七～§三十，这里只留指针）：
...
> （改成逐条上报，全量时状态条上也在动）、**全量校验跑到一半整个进程崩**
> （空闲连接池无上限 × Windows select 的 512 fd，见 lessons §二十七）、
> 崩溃/强杀后任务永远停在 running（服务启动时收尾，见 lessons §二十八）、
> **超时/异常不再让源永远显示「未校验」**（照写但不复用，见 lessons §二十九）、
> **DNS 失败区分「域名注销」与「解析被污染」**（两个来源同意才判死）、
> CLI 归因与校验链路对齐（新增「需翻墙」桶）、`diagnose --only-dead` 原本是空操作、
> `checks` 每源只留最近一条（原来只增不减，积压 6626 行）、
> 新增 `dups`：按**行为字段**找重复源（地址相同才算重复的老口径抓不到署名变体）、
> **证书错误单独一档**、`error` 带上底层异常类名（见 lessons §三十）、
