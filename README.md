...
校验只有**一台引擎**（本机引擎 = 在 JVM 里跑「阅读」App 的真源码，2026-09-20 起本地
引擎不再参与校验）。它的结论落在两处：健康档位 / 星级 / 深度进 `checks` 表（并记下
`engine` = 谁判的），逐段明细（命中的书、章数、字数、渲染与登录态）进管理库
meta（`jvm_check:<批次>:<URL>`）——同一台引擎的摘要与明细。跑完一次跑批，健康档位、
星级、列表上的「验证」列都会跟着更新（`checks.engine` 记着这一行的结论是谁判的）。
...
五档按**下一步动作**划分（两档该不该合成一档，看动作是否相同）——**动作表在
