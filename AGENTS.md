...
10. **文档也只有一个事实来源：一处写，别处只留指针。**
   ...
   - **README 与 WORKFLOW 的分工**：README = 是什么 / 怎么装 / 接口与配置 **reference**；
     WORKFLOW = 怎么用 / **口径**（健康五档、星级、深度、缓存 TTL、归因）。同一张表、
     同一份清单、同一句告诫**只在一处维护**，另一处留一行指针。
     判据是实测的：两侧逐字重复的 5 组里，**腐烂只发生在一侧**
     （`uv pip install -r requirements.txt` 在一侧写着、另一侧没有这个文件），
     而读者无从判断该信哪一份。
...
21. **取值规则末段是「属性名或取值动作」，不是选择器**（`_is_attr_or_action_name`）。
    ...
    **反过来同样成立**：拿 `getElements` 去跑一条**取值规则**（末段是动作或属性）
    ...
    就得先按末段判掉动作（只认那五个词；属性名与标签名同形，不许猜）。正文规则里
    这类占了 **2813/3794（74.1%）**。
    见 lessons §五十八、§七十五。
...
23. **注释只写「代码里读不出来的东西」**——**进度、阶段名、实测数字、「还没做」一律不写**。
    判据三问：① 删掉它，下一个人还能不能写出同一行代码？能 → 删（复述型，信息量为零）；
    ② 它讲的是**代码里的事**还是**代码外的事**（上游/平台语义、必需顺序、踩过的坑）？
    只有后者值得写；③ 它**有没有保质期**？有 → 挪去 TODO/lessons，注释里只留一行指针
    ——那里本来就该过期（它们是历史快照），而贴在代码旁边的一过期就是**误导**。
    为什么立这条：一次只读审计在**三天内**的改动里翻出 **8 处**与代码矛盾的现在时断言
    （`A1 尚未支持`、已删符号 `keepBrowserOpen`、「默认走 Gradle」、`CACHE_VERSION 13`、
    「D2 的题」、实测秒数）——**全是状态性注释**，而它们会把整段注释的可信度一起拖下水。
    **改完顺手删掉被它取代的旧叙述**：两段注释自相矛盾比没有注释更糟。
    两条边界：**约束型该长就长**（不随代码腐烂，删了等于让人重踩一遍）；**注释不是文案**
    （内部术语精确优先，机检跳过注释，别为过检改口语，见 #18）。见 lessons §七十一。
