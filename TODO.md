...
| 阶段 | 做什么 | 依赖 | 完成判据 |
|---|---|---|---|
| **S5-A JVM 调试通道** | 接替「连 App 调试」，含 cookie 注入——方案见 §一点八。**A1/A2/A3-1/A4 + D0/D1/D2 全部完成**（2026-09-20，机制见 lessons §五十九/§六十/§六十三/§六十四/§六十五/§六十六/§六十七）：**通道已接进产品，界面默认「本机引擎」，调试已走常驻（第二次 ~1s）**；**剩 D3（可选：跑批也走常驻）**；A3-2 按需（只覆盖 12 条 auth 源） | S1–S3 ✅ | §一点八 的判据 |
...
> 直起那条另有 `scripts/jvm_debug_direct.py`（D0，见下）。
>
...
**第二期：常驻 daemon（方案 2026-09-20 定；D0 同日已完成、判据达成）**
...
**D0 实测（2026-09-20，echo 源；机制与坑见 lessons §六十五）**：
...
**判据达成 → 做**：**①−② 只有 2.0s**，贵的不是 Gradle 而是 JVM + Robolectric + App
初始化那 10.7s——正是 daemon 能留住的那一段（原判据里的 ③ 要等 daemon 出来才量得到，
D0 用 ②b 当它的代理量；②b ≥ 5s 即做）。直起产物：`scripts/jvm_debug_direct.py`
（`--refresh` 生成 dump / `--measure` 打三个数 + 与 bat 的对拍表），dump 开关是环境变量
（不设就与现在完全一样）。
...
手法）；D0 已经把这条缝留好了（`run_jvm_debug(launcher=...)`），`tests/test_jvm_debug*.py`
那几条继续守。

| 批 | 做什么 | 验收判据 |
|---|---|---|
| **D0 拿到测试 JVM 的环境** ✅ 2026-09-20 | `legado-test.init.gradle` 的 `doFirst` 把 classpath / 系统属性 / jvmArgs dump 成 JSON（环境变量开关）；`java @argfile` 直起 | **不起 Gradle 也能跑完一次调试**：同一源与 bat 路径**逐字段一致**（退出码/cookie_len/每段 ok/事件数/kind 序列，`--measure` 打这张表）✅ 入场费 10.7s ✅ |
| **D1 常驻 + 单请求** ✅ 2026-09-20 | `DebugServiceDaemon`（`127.0.0.1` + 一行 JSON 请求/应答、**串行**、空闲退出、`op=ping/stop`）+ 启动器；`core/jvm_daemon.py` 客户端（拉起/探活/版本键/**回落**）；`DebugService` 拆出 `runOnce(cfg)` 供一次性与常驻共用 | 实测：各起一次 JVM 9.7/9.5s，常驻 **9.3 / 0.8s**；常驻 #2 与新 JVM #2 **逐字段一致**、两次之间也不串味；第二次 ≤2s ✅（机制与三个坑见 lessons §六十六） |
| **D2 后端接上 + 无感** ✅ 2026-09-20 | 默认拉起方式改成「**优先常驻、失败回落 Gradle**」（`core.jvm_debug.default_launcher`）；dump 比 `.kt` 旧就不走常驻（`_run_launcher` / `_run_gradle` 每次 Gradle 都顺手刷 dump，见 lessons §六十七）；回落痕迹写进第一条分段的附注 | 实测：冷启 8.8s → **第二次 1.0s**；**硬杀 daemon 后下一次自动拉起**（10.5s、退出码 0，不是报错）；改了 `.kt` 后走 Gradle（先编译）→ 再下一次 0.9s ✅ 三个数都在 lessons §六十七 |
| **D3（可选）** | 跑批也走常驻（**另一实例**）；一次并发多源 | 跑批结论与现在逐字段一致 |

> **D2 顺带定的那件事**：浏览器 profile 独占的问题（见上）**不加「让出 profile」的指令**
> ——每请求收掉浏览器只贵 **0.65s**（实测 0.7s vs 0.05s），而「让出一层协议」要两边都改、
> 还得处理「跑批忘了让出」的情况。**这个取舍记在这儿：真要拿回那 0.65s 再说。**
